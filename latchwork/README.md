...
- **本地双人**：铜匠 WASD（W 跳），铜绿 方向键（上跳）。手机左右各一块触控十字，上/下都是跳。
- **联网**：一人点「创建房间」，把四位口令（或带 `?room=` 的链接）发给同伴；同伴点「加入房间」。
- **R** 重开本关，**Esc** 回菜单。进度记在这台浏览器里，下次从上次的工台接着开。

规则：侧视跳跃。熔池只载铜匠、冷水只载铜绿，绿沼谁踩谁死（整关重来，进度还在）。踏板有人或箱子压着才顶住门闩；对钥格要两人同时站上；各自走进门洞即过关。
